...
Konstitusi global untuk Claude Code di project ini (kembar dengan `AGENTS.md` untuk Antigravity/Codex).
Detail implementasi per domain ada di skill (`.claude/skills/`). File ini = aturan wajib, skill = how-to.
...
> Untuk Antigravity CLI / Codex: pakai `AGENTS.md` di root (isi identik).
